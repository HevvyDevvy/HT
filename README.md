...
1. **Create a free Unity ID** at `id.unity.com` (phone browser, just an
   email/password signup) if you don't already have one.
2. **Add two GitHub secrets**: repo → Settings → Secrets and variables →
   Actions → add `UNITY_EMAIL` and `UNITY_PASSWORD` (your Unity account
   credentials — Personal license activates automatically from these).
3. **Run the bootstrap workflow once**: Actions tab → "Bootstrap Unity
   Project" → Run workflow. This runs Unity headlessly on GitHub's servers
   to generate the `ProjectSettings/` folder our repo is missing (since only
   `Assets/Scripts` existed before), then commits it back automatically.
   This is a one-time step — never needs running again after it succeeds.
4. **Run the build workflow**: Actions tab → "Unity Multi-Platform Build" →
   Run workflow (or just push a change to `client-unity/Assets` — it
   triggers automatically).
5. **Download the Android build**: once the `Android` job finishes, open it
   in the Actions run, download the `Build-Android` artifact (a zip
   containing the `.apk`), unzip it on your phone.
6. **Install it**: enable "install unknown apps" for whichever app you used
   to open the file (Settings → Apps → Special access, on most Android
   phones), then tap the `.apk` to install and play it — a real build of
   the actual game, entirely produced without a desktop computer.

**Being honest about risk here**: I can't run this myself to confirm it
works first try — my own environment has no Unity and no network access.
The bootstrap step in particular (`bootstrap-unity-project.yml`) is the
most likely place to hit a snag (Unity version availability, timing), and
if it fails, the error log in the Actions tab will say why — screenshot it
